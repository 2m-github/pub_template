...
// copy coding list
async function cdlindex() {
  return src(paths.cdl.index.src)
    .pipe(dest(paths.cdl.index.dest))
}
// copy coding list folder
async function cdlfolder() {
  return src(paths.cdl.folder.src)
    .pipe(dest(paths.cdl.folder.dest))
}
...
const web = parallel(fonts, images, scss, csscopy, scripts, htmlssi, cdlindex, cdlfolder, cdlguide);
...
exports.cdlindex = cdlindex;
exports.cdlfolder = cdlfolder;
